...
### 12. [프로토타입 이후 추가 개발 요구사항](#12)
### 13. [프로토타입 이후 추가 개발작업 일정](#13)
...
### 모바일 관련
- 모바일 이용 시 플레이 방법을 모바일에 맞추게 변경되는 시스템 추가
- 모바일 이용 시 오른쪽에서 위, 아래 방향 버튼이 있으며, 버튼을 누르면 캐릭터가 그에 맞춰 움직인다.
- 모바일 이용 시 왼쪽 상단에 히트 박스 On/Off 버튼이 있으며, 버튼 안에는 세로 직사각형의 모양이 새겨져 있다.
- 모바일 이용 시 왼쪽 하단에 공격 버튼이 있으며, 3 스테이지 이상에만 출력되며, 버튼 안에는 알파벳 A가 새겨져 있다.
...
- ~~플레이어의 라이프가 최대 목숨보다 낮을 경우, 회복 가스를 1개 이상 보유할 경우, 1키를 누르면 회복 가스 보유 수가 1개 줄고, 라이프를 1개 회복한다. (100%)~~
...
- ~~스테이지에서 나오는 장애물들의 모양 및 설명을 스테이지 선택 옆에 두고, 장애물 그림 위에 해당 장애물의 설명을 넣는다. ( 이름, 데미지, 특이사항 등 ) (100%) ~~
...
- ( 수정 사항 )~~보스의 체력은 250으로 정한다. (100%)~~
- ~~보스가 투사체에 맞으면 현재 체력에서 1을 뺀다. (100%)~~
- ~~( 수정 사항 )보스는 공격 모드에서 두 가지 공격 쿨다운이 있다. 두 공격 쿨다운은 0이 되면, Y좌표 3개 중 하나에 약화된 레슈 물질을 화면 중앙 X좌표에서 소환하는 특징이 있다. 하나는 보통 장애물과 속도가 같고, 하나는 빠르다. (100%)~~
- ~~보스가 사망 시 프로세스 바는 100%이기 때문에 보스 사망 애니메이션을 보여주고 사망하게 된다. (100%)~~
...
추가 사항
- ~~보스의 사망 애니메이션은 점차 부분부분 사라지는 모션으로 하며, 마지막에는 레슈 물질의 파괴 애니메이션을 넣는다. (100%)~~
...
# [ 프로토타입 이후 추가 개발 요구사항 ]<a name='12'></a>

### 1) 1주차 테스트 의견 종합 및 추가 사항
- ~~(피드백)스테이지의 우주 배경을 연속적인 그림으로 지나가게 수정 (100%)~~
- ~~(피드백)공격이 SPACE키를 누르면 키를 뗄 때까지 계속 나가게 수정 (100%)~~
- ~~(피드백)난이도가 더 어려웠으면 좋겠다 (100%)~~
- ~~1. 노말 난이도를 만든다. 노말 난이도에서는 모든 장애물의 데미지가 + 1가 된다. (100%)~~
- ~~2. 하드 난이도를 만든다. 하드 난이도에서는 모든 장애물의 속도가 2배 빨라진다. (100%)~~
- ~~3. 매드 난이도를 만든다. 매드 난이도에서는 모든 장애물의 데미지가 + 2가 된다. (100%)~~
- ~~4. 루나틱 난이도를 만든다. 루나틱 난이도에서는 모든 장애물의 데미지가 + 1가 되며 속도도 빨라진다. (100%)~~
- ~~5. 레슈 난이도를 만든다. 루나틱 난이도에서는 모든 장애물의 데미지가 + 2가 되며 속도도 빨라진다 (100%)~~
- ~~(피드백)게임 결과창에 리플레이를 넣어서 스테이지 다시 도전하게 해주면 좋겠다. (100%)~~
- ~~(피드백)응축된 레슈 물질의 색을 연하게 하면 좋겠다. (100%)~~

### 2) 2주차 7,8 스테이지 추가 제작
- ~~7~12 스테이지는 화살표를 넣어서 그것을 누르면 버튼이 나오게 만든다. (100%)~~
- ~~7 스테이지부터는 추가적으로 악화된 레슈가 나온다. (100%)~~
- ~~8 스테이지부터는 추가적으로 자폭 레슈가 나온다. (100%)~~
- ~~악화된 레슈는 다른 레슈보다 빠른 속도를 가지고 있다. (100%)~~
- ~~자폭 레슈는 다른 레슈들과 다르게 소용돌이 형태를 하고 있으며, 공격 받으면 플레이어가 데미지를 받는다. (100%)~~
- ~~7 스테이지는 지정된 Y 좌표는 3개다.(100%)~~
- ~~8 스테이지는 지정된 Y 좌표는 2개다.(100%)~~
- ~~(피드백)난이도가 더 어려웠으면 좋겠다 (100%)~~
- ~~1. 노말 난이도를 만든다. 노말 난이도에서는 모든 장애물의 데미지가 + 1가 된다. (100%)~~
- ~~2. 하드 난이도를 만든다. 하드 난이도에서는 모든 장애물의 속도가 2배 빨라진다. (100%)~~
- ~~3. 매드 난이도를 만든다. 매드 난이도에서는 모든 장애물의 데미지가 + 2가 된다. (100%)~~
...
- ~~9 스테이지부터는 추가적으로 안정된 레슈가 나온다. (100%)~~
- ~~안정된 레슈는 두 번 공격 받아야 사라진다. (100%)~~
- ~~10 스테이지에는 희미한 레슈 외의 모든 장애물이 나온다 (100%)~~
- ~~9 스테이지는 지정된 Y 좌표는 2개다.(100%)~~
- ~~10 스테이지는 지정된 Y 좌표는 3개다.(100%)~~
- ~~움직이는 레슈 물질이 나오고 설정되는 쿨다운을 0.1초 길게 한다.(100%)~~
- ~~현재 난이도의 스테이지 스코어, 전체 난이도의 스테이지 스코어를 스테이지 선택창 오른쪽에 반투명한 스테이지 스코어 이미지 위에 글자로 표시하게 한다.~~

### 4) 4주차 11 스테이지 추가 제작
-~~( 수정 사항 ) 스테이지 6에서는 스테이지 5 이전에 나온 모든 장애물이 나온다 (100%)~~
- ~~보스는 이무기의 형태를 띄고 있으며, 금 간 돌의 날개를 가지고 있다. 또한, 등은 불타고 있으며 흰색의 외눈을 가지고 있다. (100%)~~
- ~~스테이지 11에서는 자폭 레슈, 희미한 레슈 외의 모든 장애물이 나오며, 보스가 등장하며, 레일의 갯수는 2개다.(100%)~~
- ~~스테이지 11은 진행 바가 보스의 체력으로 표기되며, 외형은 같지만 시간에 따라서 진행바가 올라가는 것이 아닌 보스가 플레이어의 투사체에 맞은 횟수에 따라 정해진다. (100%)~~
- ~~보스는 이동 모드, 공격 모드 두 가지를 가지고 있으며, 정해진 유지 시간 동안 한 가지 모드를 유지하고, 유지 시간이 0이 되면 다른 모드로 전환한다. (100%)~~
- ~~보스는 이동 모드에서는 이동 쿨다운이 0이 되면, 다른 레일로 이동한다. (100%)~~
- ~~보스는 공격 모드에서는 고개를 위로 든 상태가 되며, 날개의 테두리가 검은색에서 회색으로 변한다. (100%)~~
- ~~보스의 체력은 250으로 정한다. (100%)~~
- ~~보스가 투사체에 맞으면 현재 체력에서 1을 뺀다. (100%)~~
- ~~보스가 이동 모드에서 공격 모드로 변하기 3초 전에 레이저 공격을 할 레일에 경고를 한다.(100%)~~
- ~~보스의 레이저 공격은 공격 모드 유지 시간 10초간 유지된다.(100%)~~
- ~~보스는 공격 모드에서 무작위로 정해진 레일에 레이저를 공격이 끝날 때까지 쏜다. 다른 레일에서는 무작위로 약화된 레슈, 희미한 레슈 중 하나를 0.4초~0.6초 간격으로 소환한다.(100%)~~
- ~~보스가 사망 시에는 프로세스 바는 100%이기 때문에 보스 사망 애니메이션을 보여주고 사망하게 된다. (100%)~~
- ~~11 스테이지에는 희미한 레슈가 나온다.(100%)~~
- ~~희미한 레슈는 다른 장애물과 다르게 플레이어가 피격을 받아도 데미지를 받지 않지만, 공격을 받으면 플레이어가 데미지를 받는다. (100%)~~

### 5) 5주차 12 스테이지 추가 제작
- ~~보스는 보석을 감싸는 톱니바퀴와 그것을 한꺼번에 감싸는 톱니바퀴로 되어 있다. 겉 톱니바퀴는 12개의 톱니로 되어있으며, 톱니에는 스테이지 순서대로 새롭게 나온 장애물의 색의 문양이 새겨져 있다. 안 톱니바퀴는 톱니가 8개로 되어있다.(100%)~~
- ~~스테이지 12에서는 자폭 레슈, 희미한 레슈 외의 모든 장애물이 나오며, 보스가 등장한다.(100%)~~
- ~~스테이지 12의 정보 창에서는 장애물, 레일 갯수가 나오지 않는다.(100%)~~
- ~~스테이지 12은 진행 바가 보스의 체력으로 표기되며, 외형은 같지만 시간에 따라서 진행바가 올라가는 것이 아닌 보스가 플레이어의 투사체에 맞은 횟수에 따라 정해진다. (100%)~~
- ~~보스는 공격 쿨다운이 0이 되기 3초 전에 무작위로 네 가지 패턴 중 하나를 선택하고 그 패턴을 미리 경고한다. (100%)~~
- ~~보스는 리버스, 레이저, 자폭 레슈, 레일 바꾸기 패턴이 있다. (100%)~~
- ~~레일 바꾸기를 제외한 모든 공격 패턴은 10초동안 유지된다. (100%)~~
- ~~보스가 리버스 패턴을 실행할 경우, 플레이어의 이동 방향이 반대가 되며, 플레이어의 캐릭터에게 R표시가 뜬다. (100%)~~
- ~~보스가 리버스 패턴을 실행 중이고, 공격 종료 쿨다운이 다되기 1초 전에 플레이어의 캐릭터에게 표시된 R표시가 깜빡거린다. (100%)~~
- ~~보스가 레이저 패턴을 실행할 경우, 레일 중 무작위로 한 곳에 경고 표시를 하고 1초 뒤 그 곳에 레이저를 쏜다.(100%)~~
- ~~보스가 자폭 레슈 패턴을 실행할 경우, 레일 중 무작위로 한 곳에는 희미한 레슈, 다른 곳에는 자폭 레슈를 쏜다.(100%)~~
- ~~보스가 레일 바꾸기 패턴을 실행할 경우, 레일이 세 개인 경우, 무작위로 한 곳에 있으면 사망하게 만든다. 다른 레일들은 바뀌고 난 후의 레일 그대로 이동한다. (100%)~~
- ~~레일 바꾸기 경고창에는 현재 레일, 바뀌고 난 후의 레일의 표시를 해둔다. (100%)~~
- ~~레일 바꾸기 패턴에서, 레일이 두 개일 경우, 즉사하는 레일은 없고 그냥 바뀌기만 한다. (100%)~~
- ~~보스의 체력은 1500으로 정한다. (100%)~~
- ~~보스가 투사체에 맞으면 현재 체력에서 2을 빼고, 레이저 모드일 경우에는 1을 뺀다. (100%)~~
- ~~보스가 사망 시에는 프로세스 바는 100%이기 때문에 보스 사망 애니메이션을 보여주고 사망하게 된다. (100%)~~

### 6) 6주차 다듬기
- ~~( 피드백 )레슈 골렘의 레이저를 단일이 아닌, 스파크가 튀기는 형태로 바꾼다. (100%)~~
- ~~( 피드백 )모든 스테이지 정보창의 클리어 표시가 스테이지 이름 아래에 위치하게 되며, 스테이지 스코어의 위치는 중앙에 위치하며, 테두리로 가둬진 형태로 하고 있다. (100%)~~
- ~~( 피드백 )스테이지 10의 배경이 좀 더 잘 보이는 것으로 바꾼다. (100%)~~
- ~~스테이지 12의 보스 사망 애니메이션 출력 후에 주인공 데루트와 오퍼레이터 메루스의 대화를 보여준다. 단, 이 장면은 스테이지의 첫 클리어일 경우에만 출력된다. (100%)~~
- ~~스테이지 12의 클리어 창에는 순찰에 성공했다 대신에 모든 임무를 마쳤다라는 글자가 나오게 한다.(100%)~~

### 7) 7주차 2차 다듬기 및 음향 작업
- ~~BGM과 효과음은 슬라이더를 통해서 조절이 가능하다. (100%)~~
- ~~처음 홈 씬에서 왼쪽 하단에 있는 오디오 버튼을 눌러서 오디오 팝업창을 띄우면 음향 조절이 가능하다. (100%)~~
- ~~스테이지 일시 정지 때 음향 조절이 가능하다. (100%)~~
- ~~스테이지 일시 정지 때 재시작 버튼을 추가한다. (100%)~~
- ~~홈 씬 우측 하단에 제작자와 버전 표시를 해둔다.(100%)~~

### 8) 8주차 3차 다듬기
- ~~( 버그 )스테이지 12에서 레일이 바뀔 때 차원막 조각의 위치도 바뀌지 않았던 점 수정 (100%)~~
- ~~( 버그 )게임 결과창의 재시작, 나가기 버튼이 손글씨 버튼이었던 점 수정 (100%)~~
- ~~( 난이도 조정 )난이도로 인해서 장애물들의 속도 증가량이 50%에서 40%로 변경 (100%)~~
- ~~( 난이도 조정 )난이도로 인해서 장애물의 속도가 변할 경우, 스테이지 12의 레이저 경고 시간이 짧아집니다. (100%)~~
- ~~( 피드백 )12 스테이지 보스의 레이저 패턴 중 공격 남은 시간이 1초 이하일 경우에는 레이저가 나오지 않게 수정 (100%)~~ ( 사유 : 1초 이하에서 나오는 레이저와 끝나는 동시에 나오는 장애물에 피격 받을 수 있기 때문 )

### 9) 9주차 추가적인 컨텐츠, 4차 다듬기
- ~~( 버그 )부활 가스 사용 시 저장이 안되던 버그 수정 (100%)~~
- ~~( 난이도 조정 )11 스테이지, 12 스테이지 보스의 레이저 패턴의 데미지가 1에서 2로 증가 (100%)~~
- ~~( 변경점 )플레이어의 부활 가스의 보유 수가 1개이며 부활 가스 기능이 On인 상태인 경우, 라이프가 0이 될 경우, 자동으로 부활 가스의 보유 수가 0이 되며, 게임에서 패배하지 않으며, 라이프가 1이 되며, 3초 동안 무적 상태가 된다.(100%)~~
- ~~( 변경점 )상점에는 차례로 회복 가스, 보호막 가스, 부활 가스, 인한스 가스 아이콘이 뜨며, 그 아래로는 차례대로 이름, 설명, 보유 수, 가격이 나온다.(100%)~~
- ~~부활 가스, 인한스 가스의 On/Off 기능은 스테이지 선택창에서 해단 아이템 아이콘을 클릭하는 것으로 사용 가능하다.(100%)~~
- ~~플레이어의 부활 가스 기능이 Off인 경우, 스테이지에서 부활 가스 보유 수를 0으로 한다.(100%)~~
- ~~플레이어의 인한스 가스 보유 수가 1개 이상이며, 인한스 가스 기능이 On인 상태인 경우, 스테이지 플레이 시 최대 체력이 1 늘어난다.(100%)~~
- ~~인한스 가스의 가격은 재화 200개다.(100%)~~
- ~~인한스 가스 사용 시 3번째 하트의 안쪽이 하늘색을 감싸고 있는 노란색 하트의 모습을 하고 있다. ( 안의 노란색 하트의 테두리는 흰색 )(100%)~~

### 10) 10주차 완성본 이전 마지막 추가 컨텐츠
- ~~( 변경점 )게임 음량 AUDIO를 BGM으로, EFFECT를 SFX로 표시하게 변경(100%)~~
- ~~( 변경점 )상점에는 차례로 회복 가스, 보호막 가스, 라이프 가스, 부활 가스, 인한스 가스 아이콘이 뜨며, 그 아래로는 차례대로 이름, 설명, 보유 수, 가격이 나온다.(100%)~~
- ~~( 변경점 )모든 UI를 조정한다.(100%)~~
- ~~( 변경점 )스테이지 스코어를 개별로 분류하고, 기본 UI의 버튼처럼 테두리가 있는 이미지 위에 띄울 수 있게 변경한다.(100%)~~
- ~~( 변경점 )스테이지 선택의 UI를 아이템, 재화, 스테이지 전체 클리어 표시 아이콘, 스테이지 스코어 현재 보유 수를 표시하고, 스테이지 스코어 현재 보유 수 아래에 나가기, 난이도 버튼을 둔다.(100%)~~
- ~~( 변경점 )스테이지 다음장 화살표 버튼의 크기를 줄인다.(100%)~~
- ~~( 변경점 )스테이지에서 리바이브 가스 표시를 라이프 아래에 두며, 만약 사용 가능한 상태일 경우에는 파란색의 'REVIVE : ABLE'로 표기하고, 불가능 할 경우 검은 색의 'REVIVE : UNABLE'이라고 표시한다.(100%)~~
- ~~( 변경점 )라이프의 크기를 75%로 줄인다.(100%)~~
- ~~( 변경점 )인한스 가스가 적용될 경우, 라이프 UI의 하트가 꽉 채울 정도로 4개가 표기되며, 가장 오른쪽의 하트는 인한스 가스 적용 시에 바뀌는 하트로 변경된다.(100%)~~
- ~~( 변경점 )'데미지 없음' 스테이지 스코어 조건이 '회복 아이템 사용 X'로 변경된다.(100%)~~
- ~~게임 음량 조절에 전체 음량 조절 추가(100%)~~
- ~~플레이어의 라이프가 최대 목숨보다 낮을 경우, 라이프 가스를 보유한 경우, 3키를 누르면 라이프 가스 보유 수가 1개 줄고, 라이프를 최대 목숨까지 회복한다.(100%)~~
- ~~라이프 가스의 가격은 재화 45개다.(100%)~~
- ~~해당 난이도에서 해당 스테이지의 스테이지 스코어를 모두 얻은 경우, 그 스테이지의 스테이지 명 텍스트 아래에 스테이지 스코어 아이콘을 표시한다.(100%)~~
- ~~해당 난이도에서 모든 스테이지를 클리어 한 경우, 스테이지 전체 클리어 표시 아이콘을 활성화하고, 그 위에 있던 글자는 비활성화시킨다.(100%)~~
- ~~스테이지, 스테이지 선택창의 UI에서 쉴드 가스 오른쪽에 라이프 가스를 표시하며, 그 오른쪽에는 리바이브 가스를 표시한다.(100%)~~

- ### 10) 11주차 모바일 이식 추가, 장애물 리모델링
- ~~( 변경점 )모든 파괴되는 장애물의 디자인을 별의 형태로 변경(100%)~~
- ~~( 변경점 )모든 장애물의 시간에 따른 추가 이동속도가 50%에서 33%로 변경.(100%)~~
- ~~( 변경점 )7 스테이지 이후 모든 생존 스테이지의 플레이 시간이 줄어들게 수정.(100%)~~
- ~~( 변경점 )스테이지 스코어를 개별로 분류하고, 기본 UI의 버튼처럼 테두리가 있는 이미지 위에 띄울 수 있게 변경한다.(100%)~~
- ~~( 변경점 )아이템 UI를 누르면 그 아이템을 사용할 수 있도록 변경(100%)~~
- ~~모바일 이용 시 플레이 방법을 모바일에 맞추게 변경되는 시스템 추가(100%)~~
- ~~모바일 이용 시 오른쪽에서 위, 아래 방향 버튼이 있으며, 버튼을 누르면 캐릭터가 그에 맞춰 움직인다.(100%)~~
- ~~모바일 이용 시 왼쪽 상단에 히트 박스 On/Off 버튼이 있으며, 버튼 안에는 세로 직사각형의 모양이 새겨져 있다.(100%)~~
- ~~모바일 이용 시 왼쪽 하단에 공격 버튼이 있으며, 3 스테이지 이상에만 출력되며, 버튼 안에는 알파벳 A가 새겨져 있다.(100%)~~

# [ 프로토타입 이후 추가 개발작업 일정 ]<a name='13'></a>
...
스테이지 10에서는 이전에 나온 모든 장애물들을 출현시킨다.
...
<video width="100%" height="100%" controls="controls">
  <source src="./video/week13.mp4" type="video/mp4" />
</video>
...
<img src="./image/week13-2.PNG">

수정 이전

<img src="./image/week13-3.PNG">

수정 이후

### [8주차 구현과정]
<video width="100%" height="100%" controls="controls">
  <source src="./video/week14.mp4" type="video/mp4" />
</video>
...
<video width="100%" height="100%" controls="controls">
  <source src="./video/week16-2.mp4" type="video/mp4" />
</video>

이제 모든 음량을 조절할 수 있다.

### [11주차 구현과정]
<img src="./image/week15.jpg">

이제 모바일에서 가동이 된다.
모바일에서는 오른쪽에서 위, 아래 조작이 가능하며, 왼쪽에서는 공격, 히트 박스 On/Off, 아이템을 사용할 수 있다.

<img src="./image/week15-2.png">

모든 난이도의 시간에 따른 이동 속도 증가량이 낮아졌으며, 파괴되는 장애물의 디자인이 변경되었다.
7 스테이지 이상 생존 스테이지의 플레이 시간이 줄어들었다.
