...
- 다음 스테이지 버튼을 누를경우 Stage7,8,9,10,11의 버튼이 나오며, 이전 스테이지 버튼이 있다. 이 버튼을 누르면, Stage1~6의 창이 다시 뜬다.
...
- 7 스테이지 부터는 재화가 2개 얻어지는 다른 모양의 차원막 조각이 나온다. 모습은 연푸른색의 보석 형태.
...
- 플레이어의 라이프가 최대 목숨보다 낮을 경우, 회복 가스를 1개 이상 보유할 경우, 1키를 누르면 회복 가스 보유 수가 1개 줄고, 라이프를 1개 회복한다.
...
- 플레이어의 부활 가스의 보유 수가 1개이며 부활 가스 기능이 On인 상태인 경우, 라이프가 0이 될 경우, 자동으로 부활 가스의 보유 수가 0이 되며, 게임에서 패배하지 않으며, 라이프가 1이 되며, 3초 동안 무적 상태가 된다.
