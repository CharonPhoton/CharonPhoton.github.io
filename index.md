...
|2|주인공피에젖은놈(Charactor)|<img src="./image/Charactor.png" width="300">|
|3|주인공세보턴(Charactor)|<img src="./image/Charactor.png" width="300">|
|4|진행도(Process_bar)|<img src="./image/Process_Bar.png" width="300">|
|5|체력(Health)|<img src="./image/Life.gif" width="100">|
|6|회복 가스(Healing_Gas)|<img src="./image/Healing_Gas.gif" width="100">|
|7|보호막 가스(Shield_Gas)|<img src="./image/Shield_Gas.gif" width="100">|
|8|부활 가스(Revive_Gas)|<img src="./image/Revive_Gas.gif" width="100">|
|9|차원막 파편(Dimension_Piece)|<img src="./image/Dimension_Piece.gif" width="100">|
|10|레슈 물질(ReSu)|<img src="./image/ReSu.png" width="300">|
|11|응측된 레슈 물질(UpReSu)|<img src="./image/UpReSu.png" width="300">|
|12|약화된 레슈 물질(WeakenReSu)|<img src="./image/WeakenReSu.png" width="300">|
|13|큰 레슈 물질(BigReSu)|<img src="./image/Big_ReSu.png" width="300">|
|14|움직이는 레슈 물질(MovingReSu)|<img src="./image/MovingReSu.png" width="300">|
|15|미확인 레슈 감염 괴수(Unknown_ReSu-Infested_Monster)|<img src="./image/Boss.png" width="300">|
|16|화살(Arrow)|<img src="./image/Arrow.png" width="300">|
|17|메뉴(Menu)|<img src="./image/Menu.png" width="200">|
|18|스테이지 선택(Stage_Select)|<img src="./image/Stage.png" width="200">|
|19|스테이지 최고 점수(Stage_HightScore)|<img src="./image/Score.png" width="200">|
...
- 주인공 데루트의 체력은 3이다.
- 주인공 피에 젖은 놈은 모든 것이 검고 눈만 희게 표시된 늑대인간 같은 모습에, 숄더를 장착하고 있으며, 피에 젖은 것 처럼 적갈색의 액체가 몸에 흐르는 묘사를 한다.
- 주인공 피에 젖은 놈의 공격은 레이저가 나아가서 해당 레일의 캐릭터 앞의 공격 받는 모든 것들에게 데미지를 주며, 쿨다운이 길다.
- 주인공 피에 젖은 놈의 체력은 3이다.
- 주인공 세보턴은 미래의 병사 모습을 하고 있으며, 오른팔에는 팔에 장착된 총이 있다.
- 주인공 세보턴의 공격은 큰 총알이 나아가며, 쿨다운은 조금 길다.
- 주인공 세보턴의 체력은 5이다.
- 캐릭터가 데루트만 만들어졌다면, 데루트의 체력을 일단 3으로 해둔다.
- 세보턴은 스테이지 스코어가 총 25개 이상일 때 해금된다.
- 피에 젖은 놈은 스테이지 스코어가 총 15개 이상일 때 해금된다.
- 스테이지 1,2일 경우에는 공격을 하지 못하며, 체력은 모두 공통으로 3으로 고정된다.
...
- 스테이지 7에서는 레슈 물질, 응축된 레슈 물질, 움직이는 레슈 물질, 약화된 레슈 물질, 큰 레슈 물질, 불안정한 레슈 물질이 등장한다.
- 스테이지 8에서는 레슈 물질, 응축된 레슈 물질, 움직이는 레슈 물질, 약화된 레슈 물질, 불안정한 레슈 물질이 등장한다.
- 스테이지 9에서는 레슈 물질, 응축된 레슈 물질, 움직이는 레슈 물질, 약화된 레슈 물질, 큰 레슈 물질, 불안정한 레슈 물질, 악화된 레슈 물질이 등장한다.
- 스테이지 10에서는 레슈 물질, 응축된 레슈 물질, 움직이는 레슈 물질, 약화된 레슈 물질, 큰 레슈 물질, 불안정한 레슈 물질, 악화된 레슈 물질, 보스2이 등장한다.
- 스테이지 11에서는 레슈 물질, 응축된 레슈 물질, 움직이는 레슈 물질, 약화된 레슈 물질, 큰 레슈 물질, 불안정한 레슈 물질, 악화된 레슈 물질, 보스3이 등장한다.
...
- 불안정한 레슈 물질은 고동색 + 흰색을 가지고 있으며, 투사체에 피격 되며, 약화된 레슈 보다 빠른 속도를 가지고 있다.
- 악화된 레슈 물질은 붉은색 + 흰색을 가지고 있으며, 투사체에 2번 피격되어야 파괴된다.
...
- 1 ~ 3, 8, 10 스테이지는 지정된 Y 좌표는 2개다.
- 4 ~ 6, 7, 9 스테이지는 지정된 Y 좌표가 3개다.
- 11 스테이지는 좌표가 보스 능력에 따라서 2개 혹은 3개가 된다.
- 스테이지 1~5, 7~9는 정해진 시간이 지나면 게임이 클리어가 된다.
- 스테이지 6,10,11은 진행 바가 보스의 체력으로 표기되며, 외형은 같지만 시간에 따라서 진행바가 올라가는 것이 아닌 보스가 플레이어의 투사체에 맞은 횟수에 따라 정해진다.
...
- 보스1가 투사체에 맞으면 현재 체력에서 1을 뺀다.

### 보스1 관련

- 보스1는 검은 불꽃의 형태에 흰 눈이 달려있다.
- 보스1는 무적 모드, 공격 모드 두 가지를 가지고 있으며, 정해진 유지 시간 동안 한 가지 모드를 유지하고, 유지 시간이 0이 되면 다른 모드로 전환한다.
- 보스1는 무적 모드에서는 오른쪽 끝에 화면 밖으로 1/10이 넘은 상태로 있기 때문에, 플레이어 투사체의 사거리에 닿지 않는다.
- 보스1는 무적 모드에서는 오른쪽 끝에서 유지 시간이 줄어드는 것 외에는 아무 것도 하지 않는다.
- 보스1는 공격 모드에서는 화면 중앙 기준으로 왼쪽으로 자신의 1/10이 넘은 상태로 있는다.
- 보스1는 공격 모드에서 플레이어의 투사체 사거리 범위 안에 있기 때문에 정해진 Y 좌표 3개에서 자신에게 다가오는 플레이어의 투사체에 피격당한다.
- 보스1의 체력은 250으로 정한다.
- 보스1는 공격 모드에서 두 가지 공격 쿨다운이 있다. 두 공격 쿨다운은 0이 되면, Y좌표 3개 중 하나에 약화된 레슈 물질을 화면 중앙 X좌표에서 소환하는 특징이 있다. 하나는 보통 장애물과 속도가 같고, 하나는 빠르다.

### 보스2 관련

- 보스2는 반투명 상태의 검은 유령의 형태며, 이목구비가 있지 않다.
- 보스2의 체력은 300으로 정한다.
- 보스2는 무적 모드, 공격 모드 두 가지를 가지고 있다.
- 보스2는 무적 모드에서 몸이 투명한 상태가 되며, 공격 모드에서는 몸이 보이는 형태가 된다.
- 보스2의 무적 모드에서 공격 모드로 변경될 때 레일 2개 중 하나로 랜덤하게 이동하며, 공격 모드에서는 자신이 있지 않은 레일에는 레슈 물질을 소환 속도를 빠르게 해서 소환하며, 자신이 있는 곳에서는 불안정한 레슈 물질을 느린 소환 속도로 소환한다.
- 보스2는 무적 모드에서 불투명한 약화된 레슈 물질을 소환 속도를 느린 상태에서 레일 두 개 중 하나에서 소환한다.

### 보스3 관련

- 보스3는 하얀 수정에 푸른색 갑옷을 두른 상태다.
- 보스3의 체력은 1000으로 정한다.
- 보스3은 모든 레일에 존재하기 때문에 모든 레일의 투사체에 피격을 받는다.
- 보스3은 무적 모드, 공격 모드를 가지고 있지는 않다.
- 보스3은 패턴을 4가지를 가지고 있다.
- 보스3은 첫 번째 패턴에서는 장애물들을 소환한다.
- 보스3은 두 번째 패턴에서는 플레이어가 있는 레일에 경고 표시를 뜨이고 1초 후에 그 레일에 흰 레이저를 쏴 그 곳에 있는 플레이어에게 데미지를 준다.
- 보스3은 세 번째 패턴에서 레일을 2개일 경우 3개로 바꾸고, 3개일 경우에는 2개로 바꾼다.
- 보스3의 세 번째 패턴으로 레일이 3개로 바뀔 경우, 위에 있는 플레이어는 첫 번째 레일로, 아래에 있는 플레이어는 세 번째 레일로 이동시킨다.
- 보스3의 세 번째 패턴으로 레일이 2개로 바뀔 경우, 첫 번째 레일에 있는 플레이어는 위 레일로, 세 번째 레일에 있는 플레이어는 아래 레일로, 두 번째 레일에 있는 플레이어는 위 또는 아래를 랜덤하게 반환하여 이동시킨다.
- 보스3의 세 번째 패턴 발동 시 모든 장애물들도 위와 동일하게 이동시킨다.
- 보스4의 네 번째 패턴 발동 시 게임 내 UI를 제외한 모든 오브젝트들의 위치를 좌우 반전 시킨다.
- 보스의 패턴은 랜덤하게 돌려서 결정한다.
...
- 11 스테이지의 첫 클리어 때는 엔딩을 출력하고, 엔딩에는 임무를 마치고 통로가 안정화 되었다는 장면을 추가하고 게임을 끝낸다.
- 스테이지의 선택 화면에 해당 미션의 스토리 내용 요약을 출력한다.
- 캐릭터가 어느 것으로 변경되어도 마치 스킨처럼 스토리는 데루트의 시점으로 플레이 된다.
