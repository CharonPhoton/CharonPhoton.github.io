...
- ( 변경점 )게임 음량 AUDIO를 BGM으로, EFFECT를 SFX로 표시하게 변경
- ( 변경점 )상점에는 차례로 회복 가스, 보호막 가스, 라이프 가스, 부활 가스, 인한스 가스 아이콘이 뜨며, 그 아래로는 차례대로 이름, 설명, 보유 수, 가격이 나온다.
- ( 변경점 )모든 UI를 조정한다.
- ( 변경점 )스테이지 스코어를 개별로 분류하고, 기본 UI의 버튼처럼 테두리가 있는 이미지 위에 띄울 수 있게 변경한다.
- ( 변경점 )스테이지 선택의 UI를 아이템, 재화, 스테이지 전체 클리어 표시 아이콘, 스테이지 스코어 현재 보유 수를 표시하고, 스테이지 스코어 현재 보유 수 아래에 나가기, 난이도 버튼을 둔다.
- ( 변경점 )스테이지 다음장 화살표 버튼의 크기를 줄인다.
- ( 변경점 )스테이지에서 리바이브 가스 표시를 라이프 아래에 두며, 만약 사용 가능한 상태일 경우에는 하늘색의 'REVIVE : ABLE'로 표기하고, 불가능 할 경우 검은 색의 'REVIVE : UNABLE'이라고 표시한다.
- ( 변경점 )라이프의 크기를 75%로 줄인다.
- ( 변경점 )인한스 가스가 적용될 경우, 라이프 UI의 하트가 꽉 채울 정도로 4개가 표기되며, 가장 오른쪽의 하트는 인한스 가스 적용 시에 바뀌는 하트로 변경된다.
- 게임 음량 조절에 전체 음량 조절 추가
- 플레이어의 라이프가 최대 목숨보다 낮을 경우, 라이프 가스를 보유한 경우, 3키를 누르면 라이프 가스 보유 수가 1개 줄고, 라이프를 최대 목숨까지 회복한다.
- 라이프 가스의 가격은 재화 45개다.
- 해당 난이도에서 해당 스테이지의 스테이지 스코어를 모두 얻은 경우, 그 스테이지의 스테이지 명 텍스트 아래에 스테이지 스코어 아이콘을 표시한다.
- 해당 난이도에서 모든 스테이지를 클리어 한 경우, 스테이지 전체 클리어 표시 아이콘을 활성화하고, 그 위에 있던 글자는 비활성화시킨다.
- 스테이지, 스테이지 선택창의 UI에서 쉴드 가스 오른쪽에 라이프 가스를 표시하며, 그 오른쪽에는 리바이브 가스를 표시한다.
