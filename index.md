...
- 1~3 스테이지는 지정된 Y 좌표는 2개며, 4~6 스테이지는 지정된 Y 좌표가 3개다.
- 재화는 모든 스테이지에서 나온다.
...
- 스테이지 1~5는 정해진 시간이 지나면 게임이 클리어가 되며, 스테이지 6은 진행 바가 보스의 체력으로 표기되며, 외형은 같지만 시간에 따라서 진행바가 올라가는 것이 아닌 보스가 플레이어의 투사체에 맞은 횟수에 따라 정해진다.
- 스테이지 3부터는 플레이어가 Space 키를 누르면 화면 중앙 X좌표 까지의 사거리 만큼 발사된다.
...
- 각 스테이지를 처음 클리어 하면 재화 10개를 준다.
- 플레이어의 라이프가 2 이하일 경우, 회복 가스를 1개 이상 보유할 경우, 1키를 누르면 회복 가스 보유 수가 1개 줄고, 라이프를 1개 회복한다.
- 플레이어의 보호막 가스의 보유 수 1개 이상 보유 할 경우, 장애물에 피격될 경우, 자동으로 보호막 가스의 보유 수가 1개 줄고 라이프를 잃지 않으며, 5초 동안 무적 상태가 된다.
